...
class GaussianModel:
    ...
    def get_normal(self):
        ...
        min_scale_axis = F.one_hot(torch.argmin(self.get_scaling, dim=-1), num_classes=3).float()
    ...
    def reset_tracking_opacity(self):
        ...
        if not hasattr(self, 'opacity_bank'):
            self.opacity_bank = self.get_opacity.detach()
            self.always_high_mask = self.get_opacity.squeeze() > 9999999
        else:
            self.opacity_bank = torch.cat([self.opacity_bank, self.get_opacity.detach()], dim=-1)
    ...
    def densify_and_self_split(self, grads, grad_threshold, scene_extent, N=2):
        n_init_points = self.get_xyz.shape[0]
        # Extract points that satisfy the gradient condition
        padded_grad = torch.zeros((n_init_points), device="cuda")
        padded_grad[:grads.shape[0]] = grads.squeeze()
        selected_pts_mask = torch.where(padded_grad >= grad_threshold, True, False)
        sorted_scaling = self.get_scaling.sort(1)[0]
        scale_mask = (sorted_scaling[:, 2] / sorted_scaling[:, 1].clamp_min(1e-8)) > 5
        # selected_pts_mask = torch.logical_and(selected_pts_mask,
        #                                       torch.max(self.get_scaling, dim=1).values > self.percent_dense*scene_extent)
    ...
    def densify_and_prune_split_only(self, max_grad, min_opacity, extent, max_screen_size, N=2):
        grads = self.xyz_gradient_accum / self.denom
        grads[grads.isnan()] = 0.0

        self.densify_and_self_split(grads, max_grad, extent, N=N)
...
class FlattenGaussianModel(GaussianModel):
    ...
    def reset_neighbor(self, knn_to_track=16):
        with torch.no_grad():
            ...
            knns = knn_points(self.get_xyz[None], self.get_xyz[None], K=knn_to_track, return_sorted=False)
